								...
								<tbody>
									...
									foreach($sertifikasi as $s) :
										...
										<tr>
											...
											<td>
												<?php if ($s->ssu_status == "Menunggu Pembayaran") { ?>
													<a href="<?php echo base_url('sertifikasi/form_upload_umum/' . $s->ssu_id . '/' . $s->srtu_sertifikasi .'/' . $this->session->userdata('email')); ?>" class="btn btn-warning btn-sm">Upload Bukti Bayar</a>
												<?php } else { ?>
													<?php if ($s->ssu_status == "Lunas") { ?>
													<?php } else { ?>
														<?php if($s->ssu_status == "Tolak" || $s->ssu_status == "Validasi Pembayaran") { ?>
															<a href="<?php echo base_url('sertifikasi/form_upload_umum/' . $s->ssu_id . '/' . $s->srtu_sertifikasi .'/' . $this->session->userdata('email')); ?>" class="btn btn-warning btn-sm">Upload Ulang Bukti Bayar</a>
														<?php } }
													} ?>
												</td>
											</tr>
